...
格式说明：rows 每条文案用 \\\\n 分隔意群（换段停顿）。**每行以 2-3 句为主，偶尔 1 或 4 句，严禁 5 句及以上**。
正确示例：
{{{{
...
- 🔴 **节奏变化（硬性）**：每行以 2 句或 3 句为主流节奏，偶尔 1 句或 4 句作为变化。**严禁出现 5 句及以上**
...
## 对话要求
- 角色名只能是 "A" 和 "B"（纯字母）
- 每轮对话末尾必须有【情绪标记】，用【】包裹 2-4 字，AI 自己根据语境创造，**不能全是同一长度（即不能全是2字或全是4字），至少有两种不同长度混用**
- 严禁在对话中使用 \\\\n 换行
